...
int main(int argc, const char** argv)
{
	...
	ga_model_component animated_mesh2(&bendybiped, "data/models/bendybiped_walking.fbx");
	bendybiped.set_active(false);
	bendybiped.scale(2.5);
	bendybiped.translate({ 0,0,-2 });
	sim->add_entity(&bendybiped);
...
static void gui_test(ga_frame_params* params)
{
	ga_label myLabel("Model Viewer", 20, 100, params);
	...
	if (params->_wheel_y && bendybiped.is_active()) {
		if (params->_mouse_press_mask)
			bendybiped.translate({ 0,params->_wheel_y / 5.0f , 0.0f });
		else
			bendybiped.translate({ 0, 0, params->_wheel_y / 5.0f });
	}
	...
	if (ga_button("Show Bendy Biped", 20.0f, 400.0f, params).get_clicked(params))
	{
		cube.set_active(false);
		model.set_active(false);
		model2.set_active(false);
		bendybar.set_active(false);
		bendybiped.set_active(true);
	}
