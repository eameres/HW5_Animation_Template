	...
	if (_vertex_format & k_vertex_attribute_weight)
	{
		for (int b = 0; b < mesh->mNumBones; b++)
		{
			...
			for (int w = 0; w < bone->mNumWeights; w++)
			{
				...
				*/
				///*
				if (vert->_num_influences < 4)
				{
					extern float fooCounter;
					vert->_joints[vert->_num_influences] = joint_index;
				...
				{
					for (int k = 0; k < 4; k++) {
						if (vert->_weights[k] < weight) {
							vert->_joints[k] = joint_index;
							vert->_weights[k] = weight;
							break;
						}
					}
				}
		...
		for (int i = 0; i < mesh->mNumVertices; i++) {
			ga_vertex* vert = &_vertex_array[i];
			float totalWeight = 0.0f;

			for (int k = 0; k < vert->_num_influences; k++) {
				totalWeight += vert->_weights[k];
			}
			for (int k = 0; k < vert->_num_influences; k++) {
				vert->_weights[k] /= totalWeight;
			}
		}
...
void ga_mesh::make_buffers()
{
	...
	if (_vertex_format & k_vertex_attribute_weight)
	{
		...
		// number of influencers for this vertex
		glVertexAttribIPointer(6, 1, GL_BYTE,  vertex_size, (GLvoid*)76);
